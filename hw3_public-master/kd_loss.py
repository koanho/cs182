...
class AttentionLayerLoss(nn.Module):
  ...
  def forward(self,teacher_attn,student_attn):
    ...
    loss = F.mse_loss(input=student_attn, target=teacher_attn)
    return loss
    ####################################  END OF YOUR CODE  ##################################
...
class HiddenLayerLoss(nn.Module):
    ...
    def forward(self,teacher_hddn,student_hddn):
        ...
        loss = F.mse_loss(input=proj_student, target=teacher_hddn)
        return loss
        ####################################  END OF YOUR CODE  ##################################
...
class PredictionLoss(nn.Module):
    ...
    def forward(self,teacher_pred,student_pred,t=1):
        ...
        samplewise_sce = - th.sum(target_terms * pred_terms, dim = -1)
