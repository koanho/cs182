...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        seq_pos_expanded = seq_pos[None,:,None] # 3 dim (1, seq_length, 1)
        index = seq_pos_expanded.repeat(*[1,1,self.hidden_size//2]) #(1, seq_length, self.hidden_size//2) seq_len => start부터 start+seq_len까지의 array
        ##seq_len => start가 1, sequence_length가 3이면 1,2,3

        # create the position embedding as described in the paper
        # use the `divisor` attribute instantiated in __init__ 
        # index last dim에 divisor로 나눠줌 index dim => self.hidden//2  => sin cos embedding도 self.hidden//2 가 됨.
        # stack하면 원래 dim
...
class TransformerEncoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.norm = nn.LayerNorm(input_size)
        self.self_attention = MultiHeadAttention(n_heads,[input_size,input_size]) #n_heads, [query dim, key dim]
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        norm_inputs = self.norm(inputs) #inputs should be inputs + positional embedding

        # Apply the self-attention with the normalized input, use the self_attention mask as the optional mask parameter.
        #Query, (key,value) => 2 개의 hidden_state필요
        attn = self.self_attention((norm_inputs, norm_inputs), self_attention_mask) #padding mask 추가 해야함
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):    
        ...
        #self_attention_mask: masked self attention , cross_attention_mask : padding mask
        #MultiHeadAttention.forward((query matirx, key_value matrix), mask)  
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, target_input, encoder_output, encoder_mask=None, decoder_mask=None, mask_future=False,
        ...
        # masked self attention을 위한 masking작업 => future input을 못 보게 끔
